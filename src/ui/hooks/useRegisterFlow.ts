...
import { AccountType, RegisterStep } from '../../domain/types';
...
export function useRegisterFlow(refreshSteps = false) {
  ...
  const validSteps = useCallback(async (steps: RegisterStep[]) => {
    ...
    const { stepsStatus } = await apiService.post(`auth/steps`, { stepsId });
    dispatch(
      setSteps(steps.map(
        step => ({
          ...step,
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        const { user_data } = await apiService.get("auth/user");
  ...
  const setAccount = async (type: AccountType) => {
    if (!isLogged)
      ...
      await apiService.post('auth/user', { type_account: type });
  ...
  const setEmail = useCallback(async (email?: string) => {
    ...
    const { success, message } = await apiService.post('auth/email', { type_account: getTA() });
    if (success) {
  ...
  const setPhoneOpt = useCallback(async (phone?: string) => {
    const { success, message } = await apiService.post('auth/phone', { phone });
    if (success) {
      if(phone){
  ...
  const setPassword = async (password: string, confirm_password: string) => {
    const { success, message } = await apiService.post('auth/password', { password, confirm_password });
    if (success) {
      localStorage.clear();
  ...
  const validEmailCode = async (code: string) => {
    const { success, message } = await apiService.post('auth/email/valid', { code });
    if (success) {
      navigate('/register/steps');
    } else {
      let jsonMsg;
  ...
  const validPhoneCode = async (code: string) => {
    const { success, message } = await apiService.post('auth/phone/valid', { code });
    if (success) {
      navigate('/register/steps');
    } else {
      let jsonMsg;
