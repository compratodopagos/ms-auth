...
import { phoneStep } from '../../../schemas/phoneStep';

const RegisterPhomeStep = () => {
    const { setPhoneOpt } = useRegisterFlow();
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string>();

    const handleSubmit = async ({ phone }) => {
        setLoading(true);
        const errorMsg = await setPhoneOpt(`+57${phone}`);
        if (errorMsg) {
            setError(errorMsg);
        }
        setLoading(false);
...
export default RegisterPhomeStep;