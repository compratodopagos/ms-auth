...
export function useRegisterTerms() {
    ...
    const validateTermsFreshness = useCallback(() => {
        const stored = getStoredTerms();
        if (!stored) return false;
        const now = Date.now();
        const ONE_DAY_MS = 24 * 60 * 60 * 1000;

        return Object.values(stored).every(term => {
            if (!term?.acceptedAt) return false;
            const acceptedTime = new Date(term.acceptedAt).getTime();
            return now - acceptedTime <= ONE_DAY_MS;
        });
    }, [getStoredTerms]);
