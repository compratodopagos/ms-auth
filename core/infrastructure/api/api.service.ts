...
export const apiService = {
    async get(path: string, options: RequestOptions = {}): Promise<any> {
        try {
            ...
            if(e){
                path = path.includes('?')? `${path}&e=${e}` : `${path}?e=${e}`
            }
...
const handleApiError = (path: string, error: any) => {
    try {
        ...
        if (message?.includes("Token inválido") || (message?.includes("expirado") && !message?.toLowerCase().includes('código'))) {
            localStorage.removeItem("access_token");
            window.location.pathname = "/login";
        }

        console.error(`[API ERROR] ${path}:`, message || err);
        return { success: false, message: message || details || err, requiresCaptcha };
    } catch {
