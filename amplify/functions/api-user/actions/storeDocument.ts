...
export const storeDocument = async (event: APIGatewayProxyEvent, pool: Pool, user:any): Promise<APIGatewayProxyResult> => {
    try {
        if(!user){
        ...
        const [company] = await connection.query(`SELECT id from companies where user_id = ?`,[user.id]);
        if (company.length === 0) {
        ...
        await connection.execute(`UPDATE companies SET ${inputs} WHERE id = ?`, [imageKey, company[0].id]);
        connection.release();
