...
export const getDocuments = async (event:any, pool: Pool, user:any): Promise<APIGatewayProxyResult> => {
    try {
        if(!user){
        ...
        let [documents]:any = await pool.query('SELECT document_side as type, url FROM user_documents WHERE user_id = ?', [user.id]);
