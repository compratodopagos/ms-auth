...
export const storeFile = async (event: APIGatewayProxyEvent, pool: Pool, user:any): Promise<APIGatewayProxyResult> => {
    try {
        if(!user){
        ...
        const hasFace = await validateFace(bucketName, imageKey);
        ...
        // Update
        try {
            await updateUser(connection, documentData, user);
        } catch (error:any) {
        ...
        if ((documents as any[]).length > 0) {
            ...
            await connection.execute(query, [s3Url, user.id, type]);
        } else {
            await connection.execute(query, [user.id, type, s3Url]);
        }
        ...
        return {
            statusCode: 200,
            body: JSON.stringify({ message: 'File uploaded successfully', success: true }),
        };
