...
export async function getLivenessResults(event: any, pool: Pool, poolCT: Pool, user:any): Promise<APIGatewayProxyResult> {
    ...
    if (Status == "SUCCEEDED" && (Confidence && Confidence >= 75)) {
        ...
        const [documents] = await poolCT.query('SELECT id, url, failed_attempts FROM user_documents WHERE document_side = "front" AND user_id = ?', [user.id]);
        ...
        let userDocumentUrl = `documents/${user.id}/front_id-cropped.jpg`;
        const similarity = await compareImages(userDocumentUrl, referenceImageKey, bucket, SimilarityThreshold);

        if(similarity >= SimilarityThreshold){
            await poolCT.query('UPDATE user_documents SET verified_at = NOW(), verified_url = ? WHERE id = ?', [
                referenceImageKey,
                document.id
            ]);
        } else {
            ...
            await poolCT.query('UPDATE user_documents SET failed_attempts = ?, verified_url = ? WHERE id = ?', [
                failed_attempts,
                referenceImageKey,
                document.id
            ]);
